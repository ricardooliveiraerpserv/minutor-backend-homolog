  ...
  @if(!empty($temAjustes))
    ...
    <table class="rows">
      <tbody>
        ...
        @if(!empty($adiantamentoLinhas))
          @foreach($adiantamentoLinhas as $a)
            <tr><td>Adiantamento</td><td class="muted">{{ $a['legenda'] ?: '—' }}</td><td class="right nowrap" style="color:#b91c1c;">− {{ $a['valor_fmt'] }}</td></tr>
          @endforeach
        @else
