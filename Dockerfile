...
CMD sh -c "chmod -R 775 storage bootstrap/cache && chown -R www-data:www-data storage bootstrap/cache && php artisan migrate --force && php artisan optimize 2>/dev/null || true && php artisan storage:link --force 2>/dev/null || true && supervisord -c /etc/supervisord.conf"
