...
class SkillCampaignNotifier
{
    ...
    public static function onLaunch(SkillSurvey $survey, ?User $actor = null): int
    {
        ...
        $invites = $survey->invites()->whereNull('disabled_at')->whereNotNull('user_id')->whereHas('user', fn($q)=>$q->where('enabled', true))
    ...
    public static function sweep(?Carbon $today = null): array
    {
        ...
        foreach ($campaigns as $survey) {
            $pending = $survey->invites()->whereNull('disabled_at')->whereNotNull('user_id')->whereHas('user', fn($q)=>$q->where('enabled', true))
                ->where('status', '!=', SkillSurveyInvite::STATUS_SUBMITTED)->get();
            if ($pending->isEmpty()) {
                continue;
            }
