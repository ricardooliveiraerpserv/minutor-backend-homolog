...
class TimesheetController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // Rateio de horas: se o projeto é um projeto-servidor (is_rateio), distribui as
            // horas apontadas para os projetos de destino (apontamentos-filhos billable_only).
    ...
    public function atrasos(Request $request): JsonResponse
    {
        ...
        $applyTab = function ($q) use ($tab) {
            if ($tab === 'consultor') {
                return $q->whereNotNull('late_approved_at')->where('consultant_released', false);
            }
            if ($tab === 'concluidos') {
                return $q->whereNotNull('late_approved_at')->where('consultant_released', true);
            }
            return $q->where('status', Timesheet::STATUS_LATE); // pendente
        };

        $query = $applyTab($base())
            ->with(['user:id,name', 'customer:id,name', 'project:id,name,code'])
            ->orderBy('date');
