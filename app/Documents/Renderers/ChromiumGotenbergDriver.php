...
class ChromiumGotenbergDriver implements RenderDriver
{
    ...
    public function render(string $html, array $opts = []): string
    {
        ...
        // Aguarda uma expressão JS ser verdadeira antes de imprimir (ex.: paginador do escopo).
        if (!empty($opts['waitForExpression'])) {
            $form['waitForExpression'] = (string) $opts['waitForExpression'];
        }
