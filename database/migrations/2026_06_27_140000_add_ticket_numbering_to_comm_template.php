...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('helpdesk_comm_template', function (Blueprint $table) {
            if (!Schema::hasColumn('helpdesk_comm_template', 'ticket_prefix'))   $table->string('ticket_prefix', 10)->default('');     // ex.: '' ou 'HD-'
            if (!Schema::hasColumn('helpdesk_comm_template', 'ticket_padding'))  $table->unsignedTinyInteger('ticket_padding')->default(6);
            if (!Schema::hasColumn('helpdesk_comm_template', 'ticket_sequence')) $table->unsignedBigInteger('ticket_sequence')->default(0); // último número emitido
        });
